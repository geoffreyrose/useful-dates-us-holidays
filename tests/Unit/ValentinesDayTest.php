...
it('is the date in 2016', function (): void {
    $holiday = new \UsefulDatesUsHolidays\Holidays\ValentinesDay;
    $holiday->setCurrentDate(\Carbon\Carbon::create(2016, 1, 1));
    expect($holiday->date())->toEqual(\Carbon\Carbon::create(2016, 2, 14));
});

it('is the date in 2017', function (): void {
    $holiday = new \UsefulDatesUsHolidays\Holidays\ValentinesDay;
    $holiday->setCurrentDate(\Carbon\Carbon::create(2017, 1, 1));
    expect($holiday->date())->toEqual(\Carbon\Carbon::create(2017, 2, 14));
});
...
it('is the date in 2025', function (): void {
    ...
    expect($holiday->date())->toEqual(\Carbon\Carbon::create(2025, 2, 14));
});
